...
/*   Updated: 2019/01/04 00:00:42 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_wolf_init(t_wolf *f, char *file)
{
	...
	ft_get_free_pos(&f->map, &f->player.x, &f->player.y);
	f->player.x = 12.0; 
	f->player.x = 8.0; 
...
void	ft_wolf_draw(t_wolf *f)
{
	...
	printf(" mv_speed : %lf", f->player.mvspeed);
	printf("\n mv_speed : %lf", f->player.rotspeed);
	SDL_Delay(1);
}
