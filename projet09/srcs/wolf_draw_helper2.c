...
/*   Updated: 2019/02/08 15:08:09 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_wolf_sprite(t_wolf *w)
{
	...
	while (++i < NB_SPRITES)
	{
		...
		w->sprite_screen = (int)((w->w >> 1) * (1 + w->sprite_trans.x / w->sprite_trans.y));
		...
		while (++k < w->sp_end.x)
		{
			w->tex_x = (int) ((((k - (-w->sprite_size.x / 2 + w->sprite_screen)) << 8) * TILE_WIDTH / w->sprite_size.x) >> 8);
			if (w->sprite_trans.y > 0 && k > 0 && k < w->w && w->sprite_trans.y < w->z_buffer[k])
			{
				j = w->sp_start.y - 1;
				while (++j < w->sp_end.y)
				{
					d = (j << 8) - (w->h << 7) + (w->sprite_size.y << 7);
