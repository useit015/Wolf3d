...
/*   Updated: 2019/01/05 18:32:01 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_wolf_load1(t_wolf *w)
{
	...
	w->tile[0] = ft_load_image(w->ptr.renderer, "images/eagle.bmp");
	w->tile[1] = ft_load_image(w->ptr.renderer, "images/redbrick.bmp");
	w->tile[2] = ft_load_image(w->ptr.renderer, "images/purplestone.bmp");
	w->tile[3] = ft_load_image(w->ptr.renderer, "images/greystone.bmp");
	w->tile[4] = ft_load_image(w->ptr.renderer, "images/bluestone.bmp");
	w->tile[5] = ft_load_image(w->ptr.renderer, "images/mossy.bmp");
	w->tile[6] = ft_load_image(w->ptr.renderer, "images/wood.bmp");
	w->tile[7] = ft_load_image(w->ptr.renderer, "images/colorstone.bmp");
	i = -1;
	while (++i < 8)
		if (!w->tile[i])
			ft_error();
...
int		ft_wolf_init(t_wolf *w, char *file)
{
	...
	ft_sdl_init(&w->ptr, SCR_WIDTH, SCR_HEIGHT, file);
...
void	ft_wolf_quit(t_wolf *w)
{
	...
	i = -1;
	while (++i < 8)
		SDL_DestroyTexture(w->tile[i]), w->tile[i] = NULL;
	ft_sdl_quit(&w->ptr);
}
