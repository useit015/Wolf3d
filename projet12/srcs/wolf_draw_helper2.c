...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/07 21:44:40 by ebatchas          #+#    #+#             */
/*   Updated: 2019/02/08 16:55:08 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_wolf_sprite2(t_wolf *w, int i)
{
	...
	{
		w->tex_x = (int) ((((k - (-w->sprite_size.x / 2 + w->sprite_screen)) << 8)
					* T_W / w->sprite_size.x) >> 8);
...
/*
   ...
   w->sp_end.x = w->w - 1;
   k = w->sp_start.x - 1;
   t = (w->sprite_size.y << 7) - (w->h << 7); 
   if (w->sprite_trans.y > 0) 
   while (++k < w->sp_end.x)
   {
   w->tex_x = (int) ((((k - (-w->sprite_size.x / 2 + w->sprite_screen)) << 8) * T_W / w->sprite_size.x) >> 8);
   if (k > 0 && k < w->w && w->sprite_trans.y < w->z_buffer[k])
   {
   j = w->sp_start.y - 1;
   while (++j < w->sp_end.y)
   ...
   d = t + (j << 8);
   w->tex_y = ((d * T_H / w->sprite_size.y) >> 8);
   w->color = w->tex2[w->sprite[w->sprite_order[i]].texture].pixels[T_W * w->tex_y + w->tex_x];
   if ((w->color & 0x00FFFFFF)  != 0)
