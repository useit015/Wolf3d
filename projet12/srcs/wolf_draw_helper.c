...
/*   Updated: 2019/02/08 16:17:36 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_wolf_cell_floor(t_wolf *w)
{
	...
	while (++p.y < w->h)
	{
		...
		w->pixels[p.y * w->w + p.x] = ((w->tex1[7].pixels[T_W * w->tile_floor.y
					+ w->tile_floor.x] >> 1) & 8355711);
		w->pixels[(w->h - p.y) * w->w + p.x] = w->tex1[7].pixels[T_W *
