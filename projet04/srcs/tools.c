...
/*   Updated: 2019/01/05 21:34:30 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	SDL_SetColorKey(s, SDL_TRUE, SDL_MapRGB(s->format, 0, 0, 0));
	...
	c.x = -1;
	while ((++c.x < TILE_HEIGHT) && (c.y = -1))
		while (++c.y < TILE_WIDTH)
		{
			p = (Uint8 *)s->pixels + c.x * s->pitch + c.y * 4;
			pixels[c.x * TILE_WIDTH + c.y] = *(Uint32 *)p;
		}
